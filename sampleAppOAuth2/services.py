...
from sampleAppOAuth2.models import Bearer, UpdateLog
...
def getCustomerBalance(access_token, realmId, start_date=''):
    session = Session()
    last_updated = session.query(UpdateLog).order_by('date').first()
    try:
        route = '/v3/company/{0}/reports/CustomerBalance?start_date={1}'.format(realmId, last_updated.date)
    except AttributeError as e:
        route = '/v3/company/{0}/reports/CustomerBalance'.format(realmId)
    finally:
        print(route, " - API ROUTE")
    auth_header = 'Bearer ' + access_token
    headers = {'Authorization': auth_header, 'accept': 'application/json'}
    r = requests.get(settings.SANDBOX_QBO_BASEURL + route, headers=headers)
    status_code = r.status_code
